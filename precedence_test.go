...
func TestThreeLevelPrecedence(t *testing.T) {
	...
	if s.Command != "adapter" || s.Transport != "stdio" || s.URL != "" || len(s.Headers) != 0 {
		t.Fatal("project must replace the whole global server entry")
	}
	if cfg.MCPServers["http"].Transport != "streamable-http" || cfg.MCPServers["sse"].Transport != "sse" {
		t.Fatal("Claude transport aliases not resolved")
	}
	...
	if cfg.MCPServers["shared"].Command != "zot-specific" || len(cfg.MCPServers) != 4 {
		t.Fatal("zot override or unrelated server preservation failed")
	}
	writeJSON(t, filepath.Join(project, ".mcp.json"), `{invalid`)
	cfg, err = loadConfig(project)
	if err == nil || len(cfg.MCPServers) != 0 {
		t.Fatal("invalid project must not silently use globals")
	}
}
