...
func TestManagedServerStatusCompact(t *testing.T) {
	...
	if got, want := s.status(), "grep (1 tools)"; got != want {
		t.Fatalf("ready status = %q, want %q", got, want)
	}

	s.state = stateStopped
	if got, want := s.status(), "grep (stopped)"; got != want {
...
func TestFormatStatusSummaryCompact(t *testing.T) {
	...
	if got, want := formatStatusSummary(b), "grep (1 tools) | n8n-mcp (26 tools)"; got != want {
